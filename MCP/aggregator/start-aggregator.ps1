# ============================================================
# start-aggregator.ps1 - Start RimWorld MCP Aggregator (SSE mode)
# Usage: run `.\start-aggregator.ps1` inside UESDdebuger/MCP/aggregator
#
...
# - Aggregator listens on http://127.0.0.1:3100 (upstream 3000 must be running first)
# ============================================================
...
. (Join-Path (Split-Path $scriptDir -Parent) 'resolve-runtime.ps1')
...
Write-Host ''
Write-Host 'RimWorld MCP Aggregator is starting in a new PowerShell window...' -ForegroundColor Green
Write-Host ''
Write-Host 'Log files:'
Write-Host ("  - index.js auto log : {0}\mcp-<timestamp>.log" -f $logsDir)
Write-Host "  - window tee log    : $windowLogFile"
Write-Host ''
Write-Host 'SSE endpoint : http://127.0.0.1:3100/sse'
